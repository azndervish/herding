...
```
App.js           — React components, animation, and map selector
engine.js        — Pure game engine, scenarios, and map validation
...
1. **engine.js** — pure JS rules, scenarios, procedural generation, and validation.
2. **App.js / BOARD SVG** — pure React SVG rendering components.
3. **App.js / MAP SELECTOR** — `MapSelector` screen component.
4. **App.js / APP** — animation helpers and the `App()` root component.

The engine has no React imports and is used directly by both the UI and tests.
...
- Samples every movement path at ten equal intervals
- Checks each sampled center point against impassable terrain rectangles
- Checks each interval for an exact center-path crossing of a pen wall
...
- Stops at the last valid sample when a collision is detected
...
- Center-point collision detection at ten samples per movement
- Token radius may overlap terrain, allowing large herd tokens to slip around corners
...
- Every interval between movement samples is checked for a segment crossing, so thin walls cannot be skipped
- Entities stop at the last sample before the crossing
